    ...
    def __init__(self, model_path: str):
        ...
        try:
            self.llm = ChunkedLLM(
                ...
                enforce_eager=True,
                num_kvcache_blocks=256,  # Increased for larger generations
        ...
        try:
            ...
            for output in self.llm.generate_from_chunks(
                ...
                if not output["finished"]:
                    ...
                    if new_text:
                        # Use regular print for streaming to avoid Rich buffering
                        print(new_text, end="", flush=True)

                        # Track time to first token
                        if first_token_time is None and len(new_text.strip()) > 0:
                            first_token_time = time.time() - start_stream_time
                    ...
                    cumulative_text = final_output.get("text", cumulative_text)
                    if "token_ids" in final_output:
                        token_count = len(final_output["token_ids"])

            self.console.print()  # New line after output
            ...
            if cumulative_text:
                ...
                if retained_seqs:
                    ...
                    if output_chunk_id:
                        self.state.output_chunk_ids.append(output_chunk_id)

            elapsed = time.time() - start_time

            # Filter think tags from cumulative output for display
            filtered_output = self._filter_think_tags(cumulative_text)
            ...
            self.console.print(f"\n[dim]Generated {self.state.tokens_generated} tokens in {elapsed:.2f}s ({token_count/elapsed:.1f} tok/s)[/dim]")
            if first_token_time:
                self.console.print(f"[dim]Time to first token: {first_token_time:.3f}s[/dim]")
